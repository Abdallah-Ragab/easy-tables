...
class APITable extends _Table__WEBPACK_IMPORTED_MODULE_0__.Table {
  ...
  update(data = null, options = null) {
    ...
    if (data || options) {
      this.readInput(); // this.loadingBody()

      this.loadingTable(); // setTimeout(()=>{
      //     this.renderTable()
      //     if (!this.emptyTable){
      //         this.readPrimaryHtmlElements()
      //         this.tableDataObject = this.extractDataFromHtml()
  ...
  renderTable(bodyLoading, headLoading) {
    ...
    if (bodyLoading) {
      this.tbodyHeight = this.bodyElement ? this.bodyElement.clientHeight : null;
    }

    if (headLoading) {
      this.WrapperWidth = this.tableWrapperElement ? this.tableWrapperElement.clientWidth : null;
    }

    this.tableWrapperElement.replaceChildren();

    if (this.emptyBody) {
      var tableHtml = this.constructor.htmlTemplates.emptyBodyPlaceholder;
...
class Table {
  ...
  static htmlTemplates = _templates_Table__WEBPACK_IMPORTED_MODULE_0__.tableTemplates;
...
document.querySelectorAll(`[${_modules_HTMLTable__WEBPACK_IMPORTED_MODULE_2__.HTMLTable.attributes.tableContainer}]`).forEach(wrapper => {
  wrapper.tableInstance = new _modules_HTMLTable__WEBPACK_IMPORTED_MODULE_2__.HTMLTable(wrapper);
});
